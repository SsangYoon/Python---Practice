...
while True :
    ...
    if not Data : break
    print ("First Read Way - " + Data, end = '\b')    # 읽을 파일이 이미 개행이 되어있다 그러므로 백스페이스를 한번 해주자       
...
for Line in Data_Lines : 
    print ("Second Read Way - " + Line, end = '\b')
Read_File. close()

Read_File = open ("/Users/Sangyun/Desktop/Python_Text.txt", 'r')        
Data = Read_File. read()            # 파일 객체 전체를 문자열로 반환해준다 
print ("Third Read Way - " + Data)
Read_File.close ()

# 파일을 추가하는 방법 
Append_File = open("/Users/Sangyun/Desktop/Python_Text.txt", 'a')
Append_File.write("But I`m Not Gay")
Append_File.close()

# With문과 함께 쓰기 
with open("/Users/Sangyun/Desktop/Python_Text.txt", 'r') as File : 
    print(File.read()) 

# 이렇게되면 with블럭을 벗어나는 순간 자동으로 File객체가 사리지기 때문에 
# File.close()를 할 필요가 없다 
